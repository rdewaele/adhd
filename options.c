...
void options_parse(int argc, char * argv[], struct options * options) {
	...
	strncpy(csvlogname, CSVLOGNAME, NAME_MAX);
	...
	while (-1 != (opt = getopt(argc, argv, OPTSTR))) {
		switch (opt) {
			...
			case 'I':
				...
				break;
			case 'l':
				strncpy(csvlogname, optarg, NAME_MAX);
				break;
			case 'p':
	...
	strncpy((void*)&(tmp.csvlogname), csvlogname, NAME_MAX);
